##############################################
# File can be ran as-is on TnTech HPC Server #
##############################################
# Modifications to file paths may be needed  #
...
path = '/local/tmp/jem_googlenet/'
prototxt = path + 'train_val.prototxt'
weights = path + 'type_googlenet_iter_50000.caffemodel'
...
for layer_name, layer_type in zip(net._layer_names, net.layers):
	...
	# 					os.system("ls")
	# 					os.system("caffe test --model=" + prototxt + " --log_dir=" + path + " --weights=modified_weights.caffemodel --gpu=0 2>&1 | tee log.txt")
	...
	# 							start = line.find(' = ')
	# 							end = line.find('\n')
	# 							save_file.write(str(index_num) + ',' + str(line[start+3:end]) + ',layer_name=' + str(layer_name) + ',z=' + str(z) + ',y=' + str(y) + ',x=' + str(x) + ',w=' + str(w) + ',magnitude=' + str(net.params[layer_name][0].data[z][y][x][w]) + '\n')
	...
	if layer_name == 'loss3/classifier_type':
		...
		for v in range(0,e):
			for u in range(0,f):
				...
				for line in log:
					if 'caffe.cpp:330] loss3/top-1 = ' in line:
						start = line.find(' = ')
						end = line.find('\n')
